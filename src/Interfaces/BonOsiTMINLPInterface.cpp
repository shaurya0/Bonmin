...
OsiTMINLPInterface::switchToFeasibilityProblem(size_t n,const double * x_bar,const int *inds,
    ...
    feasibilityProblem_->setAlpha(a);
...
double OsiTMINLPInterface::solveObjectiveFeasibilityProblem(size_t n,const double * x_bar,const int *inds, double alpha, double objectiveScalingFactor, double distanceScalingFactor)
{
    if(! IsValid(feasibilityProblem_)) {
        throw SimpleError("No feasibility problem","getFeasibilityOuterApproximation");
    }
     feasibilityProblem_->set_use_feasibility_pump_objective(true);
     feasibilityProblem_->set_dist_to_point_obj(n,(const Number *) x_bar,(const Index *) inds);
     feasibilityProblem_->setAlpha(alpha);
     feasibilityProblem_->setNorm(1);
     feasibilityProblem_->setObjectiveScalingFactor(objectiveScalingFactor);
     feasibilityProblem_->setDistanceScalingFactor(distanceScalingFactor);
     feasibilityProblem_->set_use_cutoff_constraint(false);
     feasibilityProblem_->set_use_local_branching_constraint(false);
     nCallOptimizeTNLP_++;
...
double
OsiTMINLPInterface::solveFeasibilityProblem(size_t n,const double * x_bar,const int *inds, int L)
{
    if(! IsValid(feasibilityProblem_)) {
        throw SimpleError("No feasibility problem","getFeasibilityOuterApproximation");
    }
    feasibilityProblem_->set_use_feasibility_pump_objective(true);
    feasibilityProblem_->set_dist_to_point_obj(n,(const Number *) x_bar,(const Index *) inds);
    feasibilityProblem_->setAlpha(0.0);
    feasibilityProblem_->setNorm(L);
    feasibilityProblem_->set_use_cutoff_constraint(false);
    feasibilityProblem_->set_use_local_branching_constraint(false);
    nCallOptimizeTNLP_++;
...
double OsiTMINLPInterface::solveFeasibilityProblem(size_t n, const double * x_bar, const int* inds, int L, double cutoff)
{
    if(! IsValid(feasibilityProblem_))
    {
        throw SimpleError("No feasibility problem","getFeasibilityOuterApproximation");
