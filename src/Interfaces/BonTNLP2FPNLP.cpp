...
TNLP2FPNLP::TNLP2FPNLP(const SmartPtr<TNLP> tnlp, double objectiveScalingFactor):
    ...
    alpha_(1.),
    norm_(1),
    objectiveScalingFactor_(0.0),
    distanceScalingFactor_(1.0),
...
TNLP2FPNLP::TNLP2FPNLP(const SmartPtr<TNLP> tnlp, const SmartPtr<TNLP2FPNLP> other):
    ...
    distanceScalingFactor_(other->distanceScalingFactor_),
...
{
    bool ret_code;
    obj_factor = objectiveScalingFactor_;
    int  nnz_obj_h = (norm_ == 2) ? (int)inds_.size() : 0;

    if(use_cutoff_constraint_ && use_local_branching_constraint_)
    {
        double coef_obj = (iRow != NULL)?0 : lambda[m - 2];
        ret_code = tnlp_->eval_h(n, x, new_x, obj_factor*alpha_ + coef_obj,
    ...
    {
        ret_code = tnlp_->eval_h(n, x, new_x, obj_factor*alpha_,
                                 m - 1, lambda, new_lambda, nele_hess - nnz_obj_h,
                                 iRow, jCol, values);
    }
    else
    { // this is the original feasibility pump implementation
        ret_code = tnlp_->eval_h(n, x, new_x, obj_factor*alpha_,
